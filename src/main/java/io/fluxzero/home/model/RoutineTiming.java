...
public sealed interface RoutineTiming {
    ...
    record Once(Instant at) implements RoutineTiming {
        public Once { java.util.Objects.requireNonNull(at); }
    ...
    record Weekly(Set<DayOfWeek> days, LocalTime time) implements RoutineTiming {
        public Weekly { days = Set.copyOf(days); java.util.Objects.requireNonNull(time); }
        public Instant nextAfter(Instant after, java.time.ZoneId zone) {
            Rules.require(!days.isEmpty(), "Choose at least one day of the week.");
