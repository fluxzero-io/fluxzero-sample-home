...
Use domain-specific details values for descriptive input and preserve focused command intent. The eight description-bearing Models and their creation/definition commands serialize at revision 1; `DetailsUpcaster` retains revision-0 history. Preserve this component registration and old fixture payloads. `docs/bestaande-opslag.md` describes the supported status transition: the original document-only Home writer also stored events. Do not republish old reports, invent baseline observations, or remove legacy documents as a shortcut.
