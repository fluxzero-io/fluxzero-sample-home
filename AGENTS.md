...
Read `README.md` and the relevant product documents in `docs/` before changing the domain. Keep this application on local SDK `2.0.0-rc.11-local.9ae3f349a2a`, built from exact commit `9ae3f349a2a94f9353e0e8419dd2ace299cc9fd7`, unless the user requests an upgrade. `scripts/prepare-sdk.sh` installs this unpublished candidate from the adjacent SDK repository without modifying that checkout. Use the work backlog in `../work-backlog` for plans and qualification notes.

For uncached candidate docs, start the plugin MCP process with `FLUXZERO_DEV_DOCS_SDK_ARCHIVE` pointing to `~/.m2/repository/io/fluxzero/fluxzero-sdk-java/2.0.0-rc.11-local.9ae3f349a2a/fluxzero-sdk-java-2.0.0-rc.11-local.9ae3f349a2a-agent-docs.zip` (expand to an absolute path). Select the exact version through `docs_start` and verify its source commit. The normal shared cache remains usable afterwards; do not silently substitute rc.11 manuals.
