...
De build importeert de gepubliceerde `io.fluxzero:fluxzero-bom:2.0.0-rc.13` uit Fluxzero Packages. De Maven Compiler voert ook de bijpassende SDK-annotationprocessor uit, zodat model- en type-indexen worden gegenereerd. De app, tests en lokale runtime gebruiken dezelfde SDK-versie; er is geen lokale SDK-build nodig.
...
| Cohesieve details | Domeinspecifieke details-value-objects met gevalideerde commandinvoer en gerichte wijzigingen die andere gegevens behouden. |
...
| Automatische commandafhandeling | Commands dragen hun eigen `@Apply`. De uitvoeringscommands handelen functionele afwijzing af als pauze; de Home Assistant-koppeling haalt vóór haar Modelcommit API-mogelijkheden op in een eigen handler. |
...
| Schrijven naar bestaande ouders | `RemoveDevice` wist een apparaat én corrigeert het primaire licht van de bestaande kamer, zonder haar ID in het command. `@Association("devices")` onderscheidt de kamer van bovenliggende ruimtes. |
...
| Doelgerichte opslag en historie | Alle Models gebruiken gewone `@Model`. Ook apparaatwaarnemingen hebben historie nodig voor het herkennen van grensoverschrijdingen. |
...
| Complete Graph-change-handlers | `RoutineSchedules` leest met `eventGraph.current()` de actuele routine en reconcilieert status en deadlines, ook na een ouder event. |
...
| Self-handling integratieberichten en WebRequestGateway | `GetHomeAssistantStates` en `CallHomeAssistantService` versturen de externe webrequest vanuit hun eigen handler. TestFixture stuurt die berichten en vervangt alleen de HTTP-responses. |
...
Zoekresultaten tonen gecommitteerde actuele documenten en zijn geen historische eventtoestand of transactionele leesafhankelijkheden. Domeinregels blijven daarom de geïnjecteerde Models/Graphs gebruiken. De scheduler vraagt met `eventGraph.current()` bewust naar de actuele routine. De SDK legt tijdens die aanroep een verse opslaggrens vast voor dezelfde Modelidentiteit; de oorspronkelijke Graph behoudt haar eventgebonden toestand. Voor reconciliatie vanaf alleen een ID blijft `loadCurrentGraph` passend. Domeinassertions blijven op de geïnjecteerde Graph om hun transactionele leesafhankelijkheden te behouden.

De versiegebonden agentdocumentatie komt via de Fluxzero-plugin uit het gepubliceerde rc.13-archief. `AGENTS.md` legt de versie en broncommit vast; de SDK-documentatie bevat de volledige contracten en uitvoerbare modelrecepten.
...
`Place` is een gedeeld domeincontract, geen zelfstandig `@Model`. `AddSpace` controleert de verplichte ouder in een kleine `@AssertLegal` via `Fluxzero.loadModel(parentId)`, dat het concrete ID-type gebruikt. De apply maakt alleen de ruimte. Er is geen eigen resolver of uitvoeringslaag.
...
De parent moet al gecommit zijn wanneer de uitvoering wordt gepland; de bestaande post-commit consumer voldoet daaraan. Annulering is asynchroon en werkt ook zonder actieve applicatieconsumer. De lokale TestServer hoort bij SDK rc.13. Een aparte gedeployde Runtime moet deze ownershipfunctie ondersteunen. `RoutineOwnershipTest` bewijst cleanup zonder routineconsumer en dat hercreatie met hetzelfde ID een oude opgeslagen uitvoering niet opnieuw geldig maakt.
...
`LinkHomeAssistantDevice` heeft een eigen `@TrackSelf`-commandhandler: hij vraagt de actuele API-mogelijkheden op met `GetHomeAssistantStates` en past daarna één Modelcommand toe. De checks voor huisgrenzen en modelbestaan blijven in de Modelpipeline. Ontdekken, polling en aflevering gebruiken dezelfde query; een serviceactie wordt een `CallHomeAssistantService`-command. Er is geen API-beaninjectie of `withBean`-fixture nodig.

`GetHomeAssistantStates` en `CallHomeAssistantService` bevatten hun eigen `@HandleQuery`/`@HandleCommand` met `Fluxzero.sendWebRequestAndWait`. Zij handelen lokaal af binnen de aanroepende workflow, zonder `@TrackSelf` of extra consumer. Ook `DiscoverHomeAssistantDevices` is een lokale self-handling query. De gewone gateway verzorgt de auditeerbare externe webrequest. `HomeAssistantEndpoint` deelt uitsluitend configuratie, requestopbouw en statuscontrole. De fixture stoot de gewone berichten aan en registreert alleen een externe HTTP-stub. Standaardcredentials blijven in hun HTTP-header, die Auditlog in zichtbare records en downloads maskeert.

Een verwachte onbereikbare installatie wordt door de interactiehandler vertaald naar `HomeAssistantUnavailable`, een herkenbare `FunctionalException`. De workflow bewaart dan de huidige fout en plant herstel. De servicebody is een concrete waarde binnen de lokale aanroep; de JSON richting Home Assistant bevat alleen `entity_id` en eventueel `brightness_pct`.

De post-commit eventconsumer levert apparaatinstellingen af. Een herpoging bevat alleen apparaat en verbinding en leest met `loadCurrentGraph` de actuele wens. Door de Device-Graph naar haar gekoppelde Model en status te navigeren blijft ook een nog afwezige of verwijderde koppeling een normale lege relatie. Bij verwijdering van een koppeling controleert `graph.current()` rechtstreeks of diezelfde Modelidentiteit nog afwezig is voordat een openstaande afleverpoging wordt geannuleerd.

`RefreshHomeAssistant` en `DeliverHomeAssistantSettings` tonen schedule-ownership voor externe systemen. Een refresh produceert complete, gewijzigde `ReportDeviceStatus`-waarnemingen, zodat de bestaande eventgebonden automatiseringen zonder leverancierslogica blijven werken. `AcceptHomeAssistantObservation` controleert de nog gekozen route vóór het rapport in dezelfde Modelpipeline wordt toegepast. Zie de [integratiehandleiding](home-assistant.md) voor fysieke aflevering, polling en de grenzen van deze SDK-versie.
