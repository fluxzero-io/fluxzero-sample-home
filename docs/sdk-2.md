...
| Uitgaande WebRequestGateway en HTTP-stubs | De Home Assistant-adapter verstuurt gewone GET/POST-webrequests met expliciete timeout, redirectbeleid en SDK-retries. TestFixture vervangt alleen de externe HTTP-responses. |
...
`LinkHomeAssistantDevice` heeft een eigen `@TrackSelf`-commandhandler: hij haalt de actuele API-mogelijkheden op en past daarna één Modelcommand toe. De echte `HomeAssistantApi` wordt via `@Autowired` als handlerparameter geleverd; de fixture gebruikt daarvoor `withBean`. De adapter maakt `WebRequest`-berichten en verstuurt ze met `Fluxzero.sendWebRequestAndWait`. Daardoor blijven HTTP-verkeer, SDK-retries en fixture-assertions op de gewone gatewayroute. Standaardcredentials staan alleen in hun HTTP-header, die Auditlog in zichtbare records en downloads maskeert. De fixture registreert afzonderlijke HTTP-stubhandlers; zij vervangt de adapter niet. De checks voor huisgrenzen en modelbestaan blijven in de Modelpipeline.
