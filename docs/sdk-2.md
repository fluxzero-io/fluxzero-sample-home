...
`GetHomeAssistantStates` en `CallHomeAssistantService` bevatten hun eigen `@HandleQuery`/`@HandleCommand` met `Fluxzero.sendWebRequestAndWait`. Hun `@TrackSelf`-annotatie en aparte consumer houden de runtimeberichtgrens zichtbaar. `HomeAssistantEndpoint` deelt uitsluitend configuratie, requestopbouw en statuscontrole. De fixture stoot de gewone berichten aan en registreert alleen een externe HTTP-stub. Standaardcredentials blijven in hun HTTP-header, die Auditlog in zichtbare records en downloads maskeert.

Een verwachte onbereikbare installatie wordt door de interactiehandler vertaald naar `HomeAssistantUnavailable`, een `FunctionalException` die ook over de berichtgrens herkenbaar blijft. De workflow bewaart dan de huidige fout en plant herstel. De concrete servicebody heeft alleen binnen het commandenvelope een subtypeaanduiding; de JSON richting Home Assistant blijft beperkt tot `entity_id` en eventueel `brightness_pct`.
