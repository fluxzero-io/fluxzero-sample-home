...
- Pending requests alongside reported settings, availability and measurements. An accepted command is an intention, not a physical acknowledgement. The default local profile has no linked equipment and displays **No report** until observations arrive. The optional [Home Assistant demo](testing-home-assistant.md) links all seven devices. Power requests are explicit: brightness never implies power on, and an unknown power state uses a left-aligned switch thumb with a question mark. Every control uses its pending request while unconfirmed, otherwise its online reported setting. Physical controls and other apps therefore update the UI after a Home request finishes. Unknown sliders show **—** without a position marker. Device tile highlighting comes only from reported online power; controls show the pending or reported setting directly. For linked devices, a **Syncing** indicator appears only after a request remains pending for one second and disappears on confirmation. A pending HTTP command uses the same delay. Delivery failures and offline state remain visible; unlinked devices retain **Not linked** instead of an endless spinner. The temperature summary pairs a thermostat setting with an online measurement from the same room, never a different room. An online device with no requested settings is not called confirmed.
...
HTTP adapters authorize the household, then dispatch existing Fluxzero commands or the `GetHomeOverview` query. They do not duplicate domain rules or call physical APIs. Device observations and pending requests are assembled from one pinned current Home graph.
